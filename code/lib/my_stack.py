...
class Stack():
    # 1. 实例化时，可使用列表或元组进行栈的赋值，并可限制栈最大长度
    def __init__(self, arr = None, maxsize = None):
        """
            栈的初始化，可以使用列表或者元组进行初始赋值，另外可以指定栈的最高高度。
        : param arr : 用来初始化的列表或者元组
        : param maxsize : 栈的最大高度
        """
        self.stack = []
